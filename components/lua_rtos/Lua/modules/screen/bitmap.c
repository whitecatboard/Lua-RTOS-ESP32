 ...
 */
int readRgb(FILE *fp, RGB *rgb, const struct display *ddisplay, int numBytes)
{
    int rc;
    UINT8 r, g, b;
    ...
    if (numBytes == 0)
	return 0;
    rc = readUINT8little(fp, &b);
    if (rc != 0)
	return rc;
    
    if (numBytes == 1)
	return 0;
    rc = readUINT8little(fp, &g);
    if (rc != 0)
	return rc;
    
    if (numBytes == 2)
	return 0;
    rc = readUINT8little(fp, &r);
    if (rc != 0)
	return rc;

    rgb->color = display_rgb(r, g, b);
 ...
 */
int readColorTable(FILE *fp, RGB *rgb, const struct display *ddisplay, int numEntries, int numBytesPerEntry)
{
    int i, rc;
    
    for (i=0; i<numEntries; i++)
    {
	rc = readRgb(fp, &(rgb[i]), ddisplay, numBytesPerEntry);
	if (rc != 0)
	    return rc;
    }
    return 0;
 ...
 */
int readBitsUncompressed(FILE *fp, RGB *image, const struct display *ddisplay, int width, int height,
			 int depth, RGB *colorTable)
{
	...
	for (row = height; row > 0; row--)
	{
		...
		for (i=0; i < ((column < 8) ? column : 8); i++)
		{
		    ...
		    image[pixel].color   = colorTable[value].color;
		    pixel++;
		}
	    }
	    if (padBytes != 0)
	...
	for (row=height; row > 0; row--)
	{
		...
		if (rc != 0)
		    return rc;
		image[pixel].color   = colorTable[temp].color;
		pixel++;
	    }
	    if (padBytes != 0)
	    {
	...
	for (row=height; row > 0; row--)
	{
	    for (column=width; column > 0; column--)
	    {
		rc = readRgb(fp, image+pixel, ddisplay, 3);
		pixel++;
	    }
	    if (padBytes != 0)
	    {
 ...
 */
int readSingleImageBMP(FILE *fp, RGB **argb, const struct display *ddisplay, UINT32 *width, UINT32 *height)
{
	...
	if (bh.size <= 12)
	    rc = readColorTable(fp, colorTable, ddisplay, numColors, 3);
	else
	    rc = readColorTable(fp, colorTable, ddisplay, numColors, 4);
	if (rc != 0)
	{
	    free(colorTable);
	    return rc;
	...
	rc = readBitsUncompressed(fp, image, ddisplay, bh.width, bh.height, depth,
				  colorTable);
	break;
    }
