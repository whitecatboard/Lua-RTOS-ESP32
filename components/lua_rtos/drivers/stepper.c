...
driver_error_t *stepper_setup(uint8_t step_pin, uint8_t dir_pin, uint8_t *unit) {
	...
	// Lock the step pin
    if ((lock_error = driver_lock(STEPPER_DRIVER, *unit, GPIO_DRIVER, step_pin))) {
    	// Revoked lock on pin
		mtx_unlock(&stepper_mutex);
	...
	// Lock the dir pin
    if ((lock_error = driver_lock(STEPPER_DRIVER, *unit, GPIO_DRIVER, dir_pin))) {
    	// Revoked lock on pin
		mtx_unlock(&stepper_mutex);
    	return driver_lock_error(STEPPER_DRIVER, lock_error);
    }
...
DRIVER_REGISTER(STEPPER,stepper,NULL,stepper_init,NULL);
