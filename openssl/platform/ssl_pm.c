...
static int mbedtls_handshake( mbedtls_ssl_context *ssl )
{
    ...
    while (ssl->private_state != MBEDTLS_SSL_HANDSHAKE_OVER) {
        ...
        SSL_DEBUG(SSL_PLATFORM_DEBUG_LEVEL, "ssl ret %d state %d", ret, ssl->private_state);
...
OSSL_HANDSHAKE_STATE ssl_pm_get_state(const SSL *ssl)
{
    ...
    switch (ssl_pm->ssl.private_state)
    {
...
int pkey_pm_load(EVP_PKEY *pk, const unsigned char *buffer, int len)
{
    ...
    ret = mbedtls_pk_parse_key(pkey_pm->pkey, load_buf, len + 1, NULL, 0, crypto_rng_wrapper, NULL);
    ssl_mem_free(load_buf);
