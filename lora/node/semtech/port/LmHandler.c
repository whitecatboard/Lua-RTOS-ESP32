...
static LmHandlerLinkCheckParams_t LinkCheckParams =
{
	.DemodMargin = 0,
	.NbGateways = 0
};
...
static void LmHandlerJoinRequest( bool isOtaa )
{
	...
	#if WHITECAT_CUSTOM_CODE
	// Force the Data Rate specifically for this request
	MibRequestConfirm_t mibGet;

	mibGet.Type = MIB_CHANNELS_DATARATE;
	LoRaMacMibGetRequestConfirm(&mibGet);

	mlmeReq.Req.Join.Datarate = mibGet.Param.ChannelsDatarate;
	#else
...
LmHandlerErrorStatus_t LmHandlerSend( LmHandlerAppData_t *appData, LmHandlerMsgTypes_t isTxConfirmed )
{
	...
	#if WHITECAT_CUSTOM_CODE
	// Get the Data Rate used for this request
	mibGet.Type = MIB_CHANNELS_DATARATE;
	LoRaMacMibGetRequestConfirm(&mibGet);
	TxParams.Datarate = mibGet.Param.ChannelsDatarate;
	#else
...
static void McpsConfirm( McpsConfirm_t *mcpsConfirm )
{
	...
	#if WHITECAT_CUSTOM_CODE
	TxParams.TxTimeOnAir = mcpsConfirm->TxTimeOnAir;
	#endif	
    ...
    case MLME_LINK_CHECK:
        {
			...
			#if WHITECAT_CUSTOM_CODE
				// Notify upper layer
				LinkCheckParams.DemodMargin = mlmeConfirm->DemodMargin;
				LinkCheckParams.NbGateways = mlmeConfirm->NbGateways;
				
				if( LmHandlerCallbacks->OnLinkCheckRequest != NULL )
				{
				    LmHandlerCallbacks->OnLinkCheckRequest( &LinkCheckParams );
				}			
			#endif
        }
        break;
