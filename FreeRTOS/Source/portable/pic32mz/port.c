...
*/
	...
	#define configCLEAR_TICK_TIMER_INTERRUPT() IFSCLR(PIC32_IRQ_T1 >> 5) = 1 << (PIC32_IRQ_T1 & 31)
#else
...
extern void *pxCurrentTCB;
	...
	IFSCLR(PIC32_IRQ_CS0 >> 5) = 1 << (PIC32_IRQ_CS0 & 31);

	/* Set software timer priority. */
	IPCCLR(PIC32_IRQ_CS0 >> 2) = 0xf << (8 * (PIC32_IRQ_CS0 & 0x03));
	IPCSET(PIC32_IRQ_CS0 >> 2) = (configKERNEL_INTERRUPT_PRIORITY << 2) << (8 * (PIC32_IRQ_CS0 & 0x03));

	/* Enable software interrupt. */
    IECSET(PIC32_IRQ_CS0 >> 5) = 1 << (PIC32_IRQ_CS0 & 31);   
	...
	xPortSupervisorStackPointer = (void *)stackptr;
	// WHITECAT END
